...
class PangleAdImpl(val t: String = "") {
    ...
    private fun postValue(si: PAGAdEcpmInfo) {
        Core.postAd(
            JSONObject().put("veronica", si.cpm.toDouble() * 1000)//ad_pre_ecpm
                .put("invalid", "USD")//currency
                .put("sony", si.adnName)//ad_network
                .put("went", "pangle")//ad_source_client
                .put("guilt", si.placement)//ad_code_id
                .put("holly", si.adUnit)//ad_pos_id
                .put("shovel", si.adFormat)//ad_format
                .toString()
        )
